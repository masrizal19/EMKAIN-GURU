  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5 flex-1 min-w-0">
        ...
        >
          {/* Sender Header for Received Messages in Group or clear identity */}
          {!isSelf && (
          ...
          {hasAttachments && (
            <div className="space-y-2">
              {msg.attachments!.map((att) => {
                ...
                const downloadUrl = (att as any).download_url || att.url;
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-1.5 px-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg text-[10px] font-black flex items-center gap-1"
                      ...
                      <a
                        href={downloadUrl}
                        download={att.name}
                        className="p-1.5 px-2.5 bg-[#FF8B7B] hover:bg-[#ff9f8f] text-gray-900 neo-border-thin rounded-lg text-[10px] font-black flex items-center gap-1 shadow-2xs"
        ...
        {/* Action Button: Delete message (Sender only) */}
        {isSelf && onDeleteMessage && (
          <div className="flex items-center justify-end gap-1 px-1">
            {showConfirmDelete ? (
              <div className="flex items-center gap-1 bg-white p-1 px-2 rounded-xl neo-border-thin text-[10px] font-bold shadow-xs">
                <span className="text-gray-700">Hapus pesan?</span>
                <button
                  onClick={() => {
                    onDeleteMessage(msg.id);
                    setShowConfirmDelete(false);
                  }}
                  className="px-1.5 py-0.5 bg-red-600 text-white rounded font-black cursor-pointer hover:bg-red-700"
                >
                  Ya
                </button>
                <button
                  onClick={() => setShowConfirmDelete(false)}
                  className="px-1.5 py-0.5 bg-gray-200 text-gray-700 rounded font-bold cursor-pointer hover:bg-gray-300"
                >
                  Batal
                </button>
              </div>
